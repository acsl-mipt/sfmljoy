...
// Headers
////////////////////////////////////////////////////////////
#include <sfmljoy/JoystickImpl.hpp>
#include <sfmljoy/Clock.hpp>
#include <sfmljoy/Err.hpp>
#include <windows.h>
#include <tchar.h>
#include <regstr.h>
#include <algorithm>
...
namespace
{
    ...
    const sfmljoy::Time connectionRefreshDelay = sfmljoy::milliseconds(500);

    ConnectionCache connectionCache[sfmljoy::Joystick::Count];
    ...
    std::string getErrorString(DWORD error)
    {
        ...
        sfmljoy::String message = buffer;
        LocalFree(buffer);
        return message.toAnsiString();
    }

    // Get the joystick's name
    sfmljoy::String getDeviceName(unsigned int index, JOYCAPS caps)
    {
        // Give the joystick a default name
        sfmljoy::String joystickDescription = "Unknown Joystick";
        ...
        if (result != ERROR_SUCCESS)
        {
            ...
            if (result != ERROR_SUCCESS)
            {
                sfmljoy::err() << "Unable to open registry for joystick at index " << index << ": " << getErrorString(result) << std::endl;
        ...
        if (result != ERROR_SUCCESS)
        {
            sfmljoy::err() << "Unable to query registry key for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
        ...
        if (result != ERROR_SUCCESS)
        {
            sfmljoy::err() << "Unable to open registry key for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
        ...
        if (result != ERROR_SUCCESS)
        {
            sfmljoy::err() << "Unable to query name for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
    ...
    for (std::vector<JoystickRecord>::iterator i = joystickList.begin(); i != joystickList.end(); ++i)
    {
        if (i->index == index)
        {
            ...
            if (!formatInitialized)
            {
                ...
                static DIOBJECTDATAFORMAT data[8 + 4 + sfmljoy::Joystick::ButtonCount];
                ...
                for (int i = 0; i < sfmljoy::Joystick::ButtonCount; ++i)
                {
                ...
                format.dwNumObjs = 8 + 4 + sfmljoy::Joystick::ButtonCount;
                format.rgodf = data;
...
BOOL CALLBACK JoystickImpl::deviceEnumerationCallback(const DIDEVICEINSTANCE* deviceInstance, void*)
{
    ...
    JoystickRecord record = { deviceInstance->guidInstance, sfmljoy::Joystick::Count, true };
...
{
    sfmljoy::priv::JoystickImpl& joystick = *reinterpret_cast<sfmljoy::priv::JoystickImpl*>(userData);
